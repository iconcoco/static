...
<template>
    <div class="calendar">
        ...
        <table cellpadding="5">
            ...
            <tbody>
                <tr v-for="(day,k1) in days" :style="`display:${k1==5?`none`:`table-row`}`" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
                    <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
                        <span :title="child.lunar" :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">{{child.day}}</span>
