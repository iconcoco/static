    ...
    <div class="fadeIn">
        <div class="block relative" v-if="flashWithdrawAD.inActice">
            <div class="banner">
                ...
                <router-link to="/login" class="cares-button-primary pain">免费领取</router-link>
            </div>
        </div>
        <div class="introduction relative">
            <router-link to="/login" class="cares-button-primary" v-if="!flashWithdrawAD.inActice">免费领取</router-link>
        </div>
...
<script>
import { getflashWithdrawNotice } from './../../../../js/api.js';
...
export default {
    ...
    created(){
        getflashWithdrawNotice({type:0}).then(res=>{
            if(res.code==0){
                if(res.data){
