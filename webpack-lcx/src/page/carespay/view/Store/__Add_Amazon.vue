...
import {
 ...
 import {
     ...
     getflashWithdrawNotice,                              //修改店铺
     bindFlashWithdraw,                              //修改店铺
     addStoreDetail,                                 //添加店铺
     enterpriseInfo
 } from '../../../../js/api.js';

//2018-11-28  实名认证的组件
import AnthenticationCompany from '../Authentication/AuthenticationCompany.vue';
        ...
        __DOAJ(){
               ...
               //4. 判断是否是编辑还是添加
                ...
                !this.isEdit && addStoreDetail(DATA).then(res=>{
                    this.ING = false;
                     if(res.code==0){
                         ...
                         switch(DATA.platformCode){
                            ...
                            //是否可以开通闪提宝服务功能
                         return getflashWithdrawNotice({type:2})
                     }else{
                         this.$Message.error(res.msg);
                     }
                 }).then(res=>{
                    if(!res) return
                    if(res.code==0){
                        res.data && res.data.isShow && (
                            bindFlashWithdraw({customerStore:this.STORE_ID[DATA.platformCode]}).then(innerItem=>{
                                !(innerItem.code==0) && this.$Message.error(innerItem.msg);
                            })
                        );
                    }else{
                        this.$Message.error(res.msg);
